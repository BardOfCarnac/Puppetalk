...
function tableUrl(id){
  ...
  url.searchParams.set('mode','controller'); url.searchParams.set('room',id); url.searchParams.set('host','1');
  return url;
}
function stageUrl(id){
  const url = new URL(location.href); url.search='';
  url.searchParams.set('mode','stage'); url.searchParams.set('room',id); url.searchParams.set('embedded','1');
  return url;
}
function joinUrl(id){
  const url = new URL(location.href); url.search='';
  url.searchParams.set('mode','controller'); url.searchParams.set('room',id);
  return url;
}
function homeUrl(){ const url=new URL(location.href); url.search=''; return url; }

function renderHome(){
  document.title='Puppetalk';
  app.innerHTML=`
    <section class="home-shell"><div class="home-panel">
      <div><div class="home-brand">Puppetalk</div><p class="home-copy">A shared puppet scene. Everyone sees the whole ensemble on their own phone while controlling their own character.</p></div>
...
function copyText(text){
  ...
  const area=document.createElement('textarea'); area.value=text; area.setAttribute('readonly',''); area.style.position='absolute'; area.style.left='-9999px';
...
function renderQr(target,invite){
  if(window.QRCode&&target) new QRCode(target,{text:invite,width:80,height:80,colorDark:'#000000',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M});
}
function wireInviteButtons(prefix,invite){
  ...
  document.querySelector(`#${prefix}-copy`)?.addEventListener('click',async()=>{
    try{await copyText(invite);feedback.textContent='Invite copied.';}catch{feedback.textContent='Could not copy the invite.';}
  });
...
function enhanceStage(id){
  const card=document.querySelector('.join-card'); if(!card)return;
  const invite=joinUrl(id).href; card.classList.add('invite-card');
  card.innerHTML=`
    ...
    <div class="invite-copy"><div class="invite-title">Scan or share to join</div><div class="invite-url">${invite}</div>
      ...
      <div class="invite-feedback" id="stage-invite-feedback"></div></div>`;
  renderQr(document.querySelector('#stage-invite-qr'),invite); wireInviteButtons('stage-invite',invite);
}
function openHostInvite(){
  if(document.querySelector('#host-invite-overlay'))return;
  const invite=joinUrl(room).href; const overlay=document.createElement('section'); overlay.id='host-invite-overlay'; overlay.className='home-shell';
  ...
  document.body.appendChild(overlay); renderQr(document.querySelector('#host-invite-qr'),invite); wireInviteButtons('host-invite',invite);
...
function addHostInviteControl(){
  ...
  const footer=document.querySelector('.controller-footer'); if(!footer)return;
  const button=document.createElement('button'); button.id='host-invite'; button.className='primary'; button.type='button'; button.textContent='Invite players'; button.style.gridColumn='1 / -1';
  button.addEventListener('click',openHostInvite); footer.prepend(button);
}
function tidyController(){
  const sub=document.querySelector('.controller-head .muted'); if(sub)sub.textContent=isHostPlayer?'Hosting · full scene':'Live table · full scene'; addHostInviteControl();
}

function bootApp(){
  let startupFailed=false;
  const onError=event=>{
    if(startupFailed)return; startupFailed=true;
  ...
  script.onload=()=>{
    ...
    if(mode==='controller')tidyController(); else enhanceStage(room);
  };
  script.onerror=()=>{
    window.removeEventListener('error',onError);
...
function bootHostPlayer(){
  app.innerHTML=`
    <section class="home-shell"><div class="home-panel">
      <div class="home-brand">Puppetalk</div><p class="home-copy">Starting the shared scene…</p>
      <div class="home-note">This phone will be Player 1 and will also host the session physics.</div>
    </div></section>`;
  const iframe=document.createElement('iframe'); iframe.src=stageUrl(room).href; iframe.title='Puppetalk session host'; iframe.setAttribute('aria-hidden','true');
  Object.assign(iframe.style,{position:'fixed',left:'0',top:'0',width:'320px',height:'360px',border:'0',opacity:'0',pointerEvents:'none',zIndex:'-1'}); document.body.appendChild(iframe);
  let started=false,polls=0;
  const startVisiblePlayer=()=>{if(started)return;started=true;clearInterval(poll);bootApp();};
  const poll=setInterval(()=>{
    polls+=1;
    try{
      const doc=iframe.contentDocument; const status=doc?.querySelector('#stage-status')?.textContent||'';
      ...
      if(hostError.includes('startup failed')||hostError.includes('could not')){clearInterval(poll);renderStartupError(`The session host could not start. ${hostError}`);return;}
