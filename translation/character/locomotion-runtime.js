// Translation-owned locomotion runtime.
(()=>{
  ...
  function stateFor(group){
    if(!states.has(group)) states.set(group,{
      slot:null,input:null,rawTorso:{x:.5,y:.6},feet:null,step:null,nextFoot:'left',walkUntil:0,stepCooldownUntil:0
    });
  ...
  function cloneInput(input){ return {...input,grabs:Array.isArray(input?.grabs)?input.grabs.map(g=>({...g})):[]}; }
  ...
  function depthForSlot(slot){ return window.PuppetalkDepthState?.getDepthForSlot?.(slot) || 0; }
  ...
  function inverseProjectedGrab(state,grab){
    ...
    return {...grab,x:clamp(center.x+(grab.x-center.x)/scale,.01,.99),y:clamp(center.y+(grab.y-shift-center.y)/scale,.02,.98)};
  }
  ...
  function inputForPhysics(state,input){
    ...
    copy.grabs=normalizedGrabs(copy).map(grab=>{
      ...
      if(grab.part==='torso') return {...mapped,y:clamp(state.rawTorso?.y ?? mapped.y,.04,.96)};
      return mapped;
    });
    if(!Array.isArray(input?.grabs)&&copy.grabs.length===1){
      const g=copy.grabs[0];
      copy.grabPart=g.part; copy.x=g.x; copy.y=g.y; copy.grabbing=true;
    }
    return copy;
  }
  ...
  function patchStageConnection(conn){
    ...
    conn.send=function(data){
      if(data?.type==='welcome'&&Number.isInteger(data.slot)){
        ...
        if(group){ slotToGroup.set(data.slot,group); stateFor(group).slot=data.slot; }
      }
  ...
  function endPoint(body,length=25){
    ...
    return {x:body.position.x-Math.sin(body.angle)*length,y:body.position.y+Math.cos(body.angle)*length};
  }
  ...
  function stageMetrics(engine){
    ...
    return {width:Math.max(320,floor.width-160),floorY:floor.body.bounds.min.y};
  }
  ...
  function footHeld(input,side){ return normalizedGrabs(input).some(g=>g.part===`${side}Foot`); }
  function beginStep(state,side,endX,floorY,now){
    ...
    state.step={side,startedAt:now,duration:420,fromX:anchor.x,toX:endX,floorY};
  ...
  function driveWalking(engine,group,bodies,now){
    ...
    if(input.rag){ state.feet=null; state.step=null; state.walkUntil=0; return; }
    ...
    if(state.step){
      ...
      stepTarget={x:lerp(state.step.fromX,state.step.toX,eased),y:state.step.floorY-Math.sin(Math.PI*t)*16};
      if(t>=1){
