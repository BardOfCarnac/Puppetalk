// Translation-owned pose tuning runtime.
(()=>{
  ...
  Peer.prototype.connect = function(...args){ return patchConnection(previousConnect.apply(this,args),'controller'); };
  ...
  function endPoint(body,length=23){
    if(!body) return null;
    const a = body.angle;
    return {x:body.position.x-Math.sin(a)*length,y:body.position.y+Math.cos(a)*length};
  }
  ...
  function pullPoint(body,point,target,stiffness,damping=.0036){
    ...
    const maxForce = .032;
    if(magnitude > maxForce){ fx *= maxForce/magnitude; fy *= maxForce/magnitude; }
  ...
  function armHeld(input,side){ return held(input,`${side}Hand`,`${side}Shoulder`); }
  ...
  function applySegmentPose(input,bodies,engine){
    ...
    if(parts.headTop && linked(engine,parts.head,parts.headTop) && !held(input,'head')) driveAngle(parts.headTop,base*.2,.013);
  ...
  function markLimp(input,bodies){
    ...
    for(const body of bodies){ body.plugin = body.plugin || {}; body.plugin.puppetalkTrueLimp = limp; }
  }
  ...
  window.PuppetalkPoseTuning = {version:25,owner:'translation'};
})();
